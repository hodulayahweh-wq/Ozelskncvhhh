...
SISTEM = {
    "apis": {}, 
    "admin_id": 7690743437,
    "ana_token": "8586246924:AAEdEGEQn9tjBBAQKw-nJ_NvDG5P-G3T8cc", # Kendi tokenini yaz
    "panel_sifre": "19786363",
    "baslangic": datetime.datetime.now().strftime("%d/%m/%Y %H:%M")
}

web_app = Flask(__name__)
web_app.secret_key = "nabi_spa_key"
...
<body style="background:#050505; color:white; font-family:sans-serif; padding:20px;">
    <div style="max-width:800px; margin:auto; background:#111; padding:30px; border-radius:20px; border:1px solid #222;">
        <h2 style="color:#0095f6">⚙️ Admin Kontrol Paneli</h2>
        ...
        <div id="list">{% for name in apis %}<div style="display:flex; justify-content:space-between; background:#0a0a0a; padding:12px; margin-bottom:8px; border-radius:8px; border-left:4px solid #0095f6;"><span>✅ {{ name }}</span><button onclick="del('{{name}}')" style="background:red; color:white; border:none; border-radius:4px; font-size:10px; padding:5px;">SİL</button></div>{% endfor %}</div>
        <br><a href="/site" target="_blank" style="display:block; text-align:center; background:#31b545; color:white; padding:15px; text-decoration:none; border-radius:10px; font-weight:bold;">🌐 VERIFIED SİTEYİ AÇ</a>
    </div>
    <script>
    function save() {
...
<head>
    ...
    <style>
        ...
        .item { font-size:24px; color:white; text-decoration:none; margin:15px; cursor:pointer; font-weight:bold; transition:0.3s; }
        ...
        input { width:100%; max-width:400px; padding:15px; border-radius:12px; border:1px solid #222; background:#0a0a0a; color:white; font-size:16px; box-sizing:border-box; }
        ...
        #res { margin-top:20px; width:100%; max-width:400px; background:#0a0a0a; padding:15px; border-radius:12px; text-align:left; font-family:monospace; color:#4ade80; display:none; border:1px solid #1a1a1a; white-space:pre-wrap; }
    </style>
</head>
<body>
    <div class="h-btn" onclick="tgl()">☰ SORGULARI GÖR</div>
    <div class="overlay" id="menu">
        <h2 style="color:var(--blue)">Nabi System<span class="tik"></span></h2>
    ...
    <div class="main">
        <h1 id="title">Hızlı Sorgu Merkezi<span class="tik"></span></h1>
...
def add_api():
    ...
    if name and url_b64: SISTEM["apis"][name] = base64.b64decode(url_b64).decode()
